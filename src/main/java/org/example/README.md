...
12. Бинарный поиск "от края"
(galloping, экспоненциальный поиск)
Небольшой недостаток классического двоичного поиска в том, 
что его сложность напрямую зависит от длины отсортированного массива. 
Если эта длина условно бесконечна, или наоборот, у нас есть основания предполагать, что искомый элемент расположен где-то в "начале",
лучше воспользоваться так называемым бинарным поиском "от края" (galloping), или экспоненциальным поиском. 
Сложность его зависит уже не от длины всего массива -- O(lg2 n), а от местонахождения искомого i-го элемента O(lg2 i).

Алгоритм поиска "от края" такой:

1. Задаём текущему индексу начальное значение. Эти значения вычисляются по формуле 2^i - 2, где i >= 1. 
Исходно i=1, и текущий индекс получит значение 0 (2^1 - 2).

2. Берём элемент с текущим индексом. Если он равен искомому, возвращаем "найден".

3. Если элемент с текущим индексом меньше искомого значения, то вычисляем следующее значение текущего индекса. 
Если новый текущий индекс меньше размера массива - 1, то переходим к п. 2. 
Иначе задаём текущему индексу значение, равное размеру массива - 1.

4. (элемент с текущим индексом больше искомого)
   Выбираем нижнюю и верхнюю границы диапазона, где, возможно, находится искомый элемент.
   Верхняя граница равна текущему индексу, а нижняя -- предыдущему значению текущего индекса (2^(i-1) - 2) + 1.

Для поиска в этом диапазоне используйте класс BinarySearch из предыдущего занятия (включите его в файл с решением) 
-- проинициализируйте рабочим массивом, задайте границы диапазона Left и Right, 
и итеративно выполняйте Step() до окончательного результата.

Задание.
Добавьте в класс BinarySearch из прошлого занятия (не изменяя его) метод GallopingSearch(), 
который получает первым параметром исходный отсортированный по возрастанию массив целых чисел int[], 
а вторым параметром -- искомое целое значение. Этот метод выполняет поиск искомого значения по алгоритму из данного занятия, 
и возвращает true/false в зависимости от того, найден ли элемент.

Решение:
BinarySearchGalloping
